...
|---|---|---|---|---|
| Strict local Qwen conversational turn | Current | Yes: `Controller`, schema-v2 validation, bounded corrective retry | Yes, typed CLI and resident/browser turn coordinator | Router task is installed but current cold-start launch is blocked by Code Integrity; current UI baseline is physically accepted |
...
| Optional 4090 execution and failover | Current optional-compute policy | Yes: health/model checks, exact MO2 status, freshness, fail-closed eligibility | **No** in ordinary CLI/resident composition | Endpoint and policy service observed live; use remains optional and gated |
