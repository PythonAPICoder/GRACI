...
from pronunciation import speech_presentation_text

SOURCE_LINES = (
    "GRACI is online and ready.",
    "Good morning. GRACI is ready to help.",
...
def main() -> None:
    ...
    for index, source_text in enumerate(SOURCE_LINES, 1):
        ...
        output = args.output / f"graci-gray-see-{index:02d}.wav"
        sf.write(output, audio, sample_rate)
        results.append({
            "line": index,
            "authoritative_source_text": source_text,
            "speech_presentation_text": speech_text,
            "pronunciation_override": {"written": "GRACI", "spoken": "GRAY-see"},
    ...
    evidence = {
        ...
        "voice": "af_bella",
        "purpose": "Phase 6A speech-only pronunciation finalist check",
        "authoritative_text_mutated": False,
        "results": results,
    }
    (args.output / "pronunciation-audition.json").write_text(
